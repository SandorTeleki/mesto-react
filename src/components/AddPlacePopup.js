...
function AddPlacePopup(props) {
  ...
  return (
    <PopupWithForm name={'add-card-form'} 
                title='Новое место' 
                isOpen={props.isOpen}
      ...
      <fieldset className={'popup__inputs'}>
        <input type="text"
               ...
               maxLength="30"
               required/>
        <span className={`popup__error ${showCardNameError&& 'popup__error_visible'}`}>{cardName.error}</span>
        <input type="url"
               className="popup__input"
               name="link"
               ...
               id="add-link"
               required/>
        <span className={`popup__error ${showCardLinkError&& 'popup__error_visible'}`}>{cardLink.error}</span>
        <button type="submit"
